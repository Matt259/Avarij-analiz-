...
def return_cleaned_df():
    file_names = ("accidents_2013","accidents_2014","accidents_2015",
                "accidents_2016","accidents_2017","accidents_2018",
                "accidents_2019","accidents_2020","accidents_2021")
        
    dict_list = []
    for index in range(len(file_names)):
        accident = JSONReader(file_names[index]+".json", int(file_names[index].split("_")[1]))
...
def plot_car_incidents_and_deaths_per_year(df, unique_years, years_from_to, to_png=False):
    ...
    data_dict = {"Avarijos": car_accidents, "Mirtys": deaths_by_year}
    
    plt.figure(figsize=(18,10))
    for label, stat in data_dict.items():
        plt.plot(unique_years, stat, marker="o", label=label)
        for x, y in zip(unique_years, stat):
